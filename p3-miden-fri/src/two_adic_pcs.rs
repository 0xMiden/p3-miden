...
/// Lagrange interpolation over an extension field.
///
/// Given base field points `xs` and extension field evaluations `ys`,
/// compute the interpolated value at the extension field point `point`.
fn lagrange_interpolate_ext<F, EF>(xs: &[F], ys: &[EF], point: EF) -> EF
where
    F: TwoAdicField,
    EF: ExtensionField<F>,
{
